#!/usr/bin/env python3
"""
Deployment preflight (fail-closed) for dual-repo server layout.

This is a STRICT non-live contract:
- live trading must be disabled
- critical imports must resolve under overlay
...
def verify_filesystem(p: Paths) -> Dict[str, Any]:
    ...
    venv_py = (p.venv_root / "bin" / "python").is_file() or (p.venv_root / "bin" / "python3").is_file()
    checks["venv_python"] = _truth(bool(venv_py), reason="venv_python_missing")
...
def build_report(p: Paths) -> Dict[str, Any]:
    ...
    return {
        ...
        "honesty": "Preflight does not validate secrets; it validates switch-ready non-live posture and wiring.",
    }
