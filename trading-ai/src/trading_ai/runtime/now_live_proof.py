"""Durable proof artifacts: now-live autonomy stack vs live venue submission (must stay split)."""
...
def write_now_live_autonomy_artifacts(*, runtime_root: Optional[Path] = None) -> Dict[str, Any]:
    """
    Writes:
    - ``data/control/full_autonomy_now_live_mode.json``
    - ``data/control/full_autonomy_live_execution_status.json``

    Fail-closed if live-trading env flags are on (this stack must not run as-is).
    """
...
def run_authoritative_live_guard_proof(*, runtime_root: Optional[Path] = None) -> Tuple[bool, Dict[str, Any]]:
    """Executable proof: autonomy may run only when live-trading env is off."""
    enforce_non_live_env_defaults()
    ok, why = assert_live_trading_env_disabled()
    root = _root(runtime_root)
