...
def _run_accelerated_sim(*, cycles: int, runtime_root: Path, skip_models: bool) -> Dict[str, Any]:
    ...
    for i in range(max(1, int(cycles))):
        ...
        for _ in range(6):
            advance_simulated_fill_once(runtime_root=runtime_root)
        if i % 3 == 0:
            time.sleep(0.01)
    ...
    verdict = {
        ...
        "simulated_fill_summary_exists": (runtime_root / "data" / "control" / "simulated_fill_chain" / "reconciliation_summary.json").is_file(),
    ...
    ok2 = (
        ...
        and verdict["simulated_fill_summary_exists"]
...
def main() -> int:
    ...
    sim.add_argument("--cycles", type=int, default=8)
